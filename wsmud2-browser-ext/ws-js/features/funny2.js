  ...
  `);
      ...
      `);
        ...
        );
        {
            $(".left-hotkeys").append(
                $("<div></div>").append(
                    $(`<hic class="span-btn"></hic>`).append("刷新").click(() => {
                        clickInfo();
                        clickPack();
                        SendCommand("events");
                    }),
                    $(`<hic class="span-btn"></hic>`).append("统计").click(() => SendCommand("info")),
                    $(`<hic class="span-btn"></hic>`).append("换组").click(checkEq),
                    $(`<hic class="span-btn"></hic>`).append("扩展").click(openExtend),
                    $(`<hic class="span-btn"></hic>`).append("回复").click(() => ToRaid.perform("@renew")),
                    $(`<hic class="span-btn"></hic>`).append("挂机").click(() => WG.zdwk()),
                    $(`<hic class="span-btn"></hic>`).append("仓库").click(() => SendCommand("store")),
                    $(`<hic class="span-btn"></hic>`).append("随从").click(dzsc),
                ),
                $("<div></div>").append(
                    $(`<hig class="span-btn"></hig>`).append("师父").click(() => SendCommand("goto fam1 WUDANG")),
                    $(`<hig class="span-btn"></hig>`).append("木人").click(() => SendCommand("$to 少林派-西侧殿")),
                    ...
                    $(`<hig class="span-btn"></hig>`).append("当铺").click(() => SendCommand("$to 扬州城-当铺;$wait 200;list {r唐楠};")),
                    $(`<hig class="span-btn"></hig>`).append("回家").click(() => SendCommand("$to 住房")),
                    $(`<hig class="span-btn"></hig>`).append("武道").click(() => SendCommand("jh fam 9 start;go enter")),
                ),
                $("<div></div>").append(
                    $(`<hiz class="span-btn"></hiz>`).append("衙门").click(() => SendCommand("$to 扬州城-衙门正厅")),
            $(`<hiz class="span-btn"></hiz>`).append("花园").click(() => SendCommand("$to 住房-小花园"))
                ),
                $("<div></div>").append(
                    $(`<hio class="span-btn"></hio>`).append("隐藏").click(hideLeftRight),
                    $(`<hio class="span-btn"></hio>`).append("清屏").click(clearRightMsg),
                    $(`<hio class="span-btn"></hio>`).append("工具").click(() => openExtensionHtml()),
                    $(`<hio class="span-btn"></hio>`).append("攻略").click(() => window.open("https://ucn595zz2fou.feishu.cn/wiki/JvEZw8bEiiIpf3kQiFJcAwbanji", "_blank")),
      ...
      `);
        ...
        function clickPack() {
            ...
            SendCommand("pack");
            setTimeout(() => $(".dialog-close").click(), 100);
    ...
    */
    ...
    function initPopupSettings(container) {
        ...
        container.find('#skillCDColor').val(getVal(rid + '_skillCDColor') || 'hir').off('change').on('change', function () {
            ...
            if (typeof window.skillCDColor !== 'undefined') window.skillCDColor = $(this).val();
        });
        container.find('#buffCDColor').val(getVal(rid + '_buffCDColor') || 'hig').off('change').on('change', function () {
            setVal(rid + '_buffCDColor', $(this).val());
            if (typeof window.buffCDColor !== 'undefined') window.buffCDColor = $(this).val();
        });
